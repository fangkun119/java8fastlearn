...
public class ComparatorsTest
{
   public static void main(String[] args) {
      ...
      Arrays.sort(TestData.presidents, Comparator.comparing(Person::getName));
      System.out.println(Arrays.toString(TestData.presidents));

      // Comparator.thenComparing与Comparator.comparing结合使用，来进行多级比较
      Arrays.sort(TestData.presidents,
         Comparator
      ...
      Arrays.sort(TestData.presidents, Comparator.comparingInt(p -> p.getName().length()));
      System.out.println(Arrays.toString(TestData.presidents));

      // nullsFirst(naturalOrder())或nullsLast(naturalOrder())来让null值排在最前或最后，而不是抛异常
      Arrays.sort(TestData.presidents, Comparator.comparing(Person::getMiddleName, nullsFirst(naturalOrder())));
      System.out.println(Arrays.toString(TestData.presidents));

      // reversed()用来颠倒任何一个比较器，例如natrualOrder().reversed()等价于reverseOrder()
      Arrays.sort(TestData.presidents, Comparator.comparing(Person::getMiddleName, nullsFirst(Comparator.<String>naturalOrder().reversed())));
      System.out.println(Arrays.toString(TestData.presidents));
   }
}
